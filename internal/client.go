...
func (c *client) readDataFromServer() {
	for {
		...
		if er != nil {
			pkg.LogError("failed read data", er)
			break
		}

		messages, err := decodeMessages(packet)
		if err != nil {
			pkg.LogInfo(string(packet))
			pkg.LogError("failed in message parse", err)
		}
...
func (c *client) Subscribe(topic string, handler MessageHandler) {
	...
	if err != nil {
		pkg.LogError("failed send message to broker server", err)
	}
}
